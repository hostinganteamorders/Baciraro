  ...
  return (
    <section className="relative border-t border-white/5 py-24 sm:py-32">
      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6 md:px-8">
        ...
        <div className="hidden md:flex items-center justify-between gap-2 mb-16">
          {steps.map((step, i) => (
            <motion.div
              key={step}
            ...
            >
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-center text-[10px] font-bold uppercase tracking-wider text-[#F4F1EA]/70 max-w-[80px]">
                  {step}
                </span>
              </div>
              {i < steps.length - 1 && (
                <div className="h-px w-8 bg-gradient-to-r from-[#34D399]/30 to-[#34D399]/10 mb-6" />
              )}
            </motion.div>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-4 md:hidden mb-12">
          {steps.map((step, i) => (
            ...
            >
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[#34D399]/30 bg-[#34D399]/10 text-[10px] font-bold text-[#34D399]">
                {String(i + 1).padStart(2, "0")}
              </span>
              <span className="text-xs font-bold uppercase tracking-wider text-[#F4F1EA]/70">
                {step}
              </span>
              {i < steps.length - 1 && (
        ...
        >
          <div className="border border-white/5 bg-white/[0.02] p-6 sm:p-8">
            ...
            <p className="mt-3 text-sm text-[#0076C8]">
